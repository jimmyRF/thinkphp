	...
	var validate = $("#signupForm").validate({
		...
		messages: {
			...
			password: {
				...
				minlength: jQuery.format($.i18n.prop("密码长度不符合")),
				maxlength: jQuery.format($.i18n.prop("密码长度不符合"))
			},
			passwordAgain: {
				required: $.i18n.prop("请输入密码"),
				minlength: jQuery.format($.i18n.prop("密码长度不符合")),
				maxlength: jQuery.format($.i18n.prop("密码长度不符合")),
				equalTo: jQuery.format($.i18n.prop("两次输入密码不一样"))
			},
			tel: {
				required: $.i18n.prop("请输入手机号码"),
				digits: $.i18n.prop("手机号码格式错误")
			},
		}
	});
		...
		if($("#checkBox").attr("checked")){
			...
			$.ajax({
				...
				data: {
					...
					member_password: md5.MD5($("#password").val()),
					member_tel: $("#tel").val(),
				...
				success: function(data){
					...
					if(data == 0){
						...
						//参数传递失败
						$(".login-error").show();
						$(".login-error").html($.i18n.prop("出错了！请联系管理员！！"));
					}else if(data == 3){
						//账号已经被注册
						
						$(".login-error").show();
						$(".login-error").html($.i18n.prop("账号已经被注册"));

					}else if(data == 5){
						//邮箱已经被注册
						$(".login-error").show();
						$(".login-error").html($.i18n.prop("邮箱已经被注册"));
					}else{
						//系统错误
