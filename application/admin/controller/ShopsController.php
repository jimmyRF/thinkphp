...
use think\Image;
use think\Request;
class ShopsController extends AdminAuth
{
	//模块基本信息
	private $data = array(
		'module_name' => '商品',
		'module_url'  => "/admin/shops/",
    ...
    public function index()
    {
        ...
        if(!empty($param)){
            ...
            if(isset($param['title'])){
                $map['shop_name'] = ['like','%'.$param['title'].'%'];
            }
        }

        $list =  Shops::where($map)
                        ->order('id', 'DESC')
                        ->paginate();

        for ($i=0; $i < count($list); $i++) { 
            $list[$i]['shop_createtime'] = date('Y-m-d', $list[$i]['shop_createtime']); 
        }
