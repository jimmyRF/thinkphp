...
class IndexController extends AdminAuth
{
    public function index()
    {
        if (isset($_SESSION['think'])) {
            ...
            $this->data['shop_count_all'] = Shops::where('shop_status','>=',0)->count();
            $this->data['shop_count_latest_month'] = Shops::whereTime('shop_createtime','>=',date('Y-m-01'))->where('shop_status','>=',0)->count();
    ...
    public function logout(){
        $request = request();
        session(null, 'think');
        return $this->success('已成功登出', $request->root(true).'/admin/login');
    }
}
