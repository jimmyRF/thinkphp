...
class AdminAuth extends Controller {
	protected function _initialize(){
		...
		if(!session('uid')){
			...
			header(strtolower("location: ".APP_PATH."/admin/view/index/login"));
            exit();
		}
	...
	*/
	protected function auth_password_check(){
        ...
        if (!$user) {
            ...
            $this->error('登录失效:用户密码已更改',$request->root(true).'/admin/login');
        }
	}
	...
	protected function auth_expired_check(){
        ...
        if ((time() > strtotime($user->expire_time))) { //登录超时
            ...
            $this->error('账号已过期',$request->root(true).'/admin/login');
        }
	}
}
